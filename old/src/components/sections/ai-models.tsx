...
export default function AIModelsSection() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const progress = Math.max(0, Math.min(1, scrollPos / (sectionHeight - window.innerHeight)));
      const newIndex = Math.min(MODELS.length - 1, Math.floor(progress * MODELS.length));
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden">
            ...
            >
              {model.src ? (
                ...
                <div className="h-full w-full bg-neutral-900 flex items-center justify-center">
                   <span className="text-neutral-700 font-mono text-sm uppercase">Asset Placeholder</span>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="absolute top-0 left-0 w-full h-[30vh] bg-gradient-to-b from-[#F1F3F2] to-transparent z-20 pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-full h-[30vh] bg-gradient-to-t from-background to-transparent z-20 pointer-events-none" />
            ...
            <div className="relative h-[200px] w-full flex flex-col items-end md:items-end justify-center">
              ...
              >
                {MODELS.map((model, idx) => (
                  <div
                    key={idx}
                    className={`h-[64px] flex items-center transition-all duration-500 whitespace-nowrap ${
                      activeIndex === idx 
